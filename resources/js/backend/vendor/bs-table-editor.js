...
class BSTable {
  ...
  _actionsModeNormal(button) {
    jQuery(button).parent().find('#bAcep').hide();
    jQuery(button).parent().find('#bCanc').hide();
    jQuery(button).parent().find('#bEdit').show();
    jQuery(button).parent().find('#bDel').show();
    var $row = jQuery(button).parents('tr');         // get the row
    $row.attr('data-status', '');               // remove editing status
  }
  _actionsModeEdit(button) {
    jQuery(button).parent().find('#bAcep').show();
    jQuery(button).parent().find('#bCanc').show();
    jQuery(button).parent().find('#bEdit').hide();
    jQuery(button).parent().find('#bDel').hide();
    var $row = jQuery(button).parents('tr');         // get the row
  ...
  // Indicate user is editing the row
    var $row = jQuery(button).parents('tr');       // access the row
  ...
  _actionAddRow() {
    ...
    if ($allRows.length==0) { // there are no rows. we must create them
      ...
      $cols.each(function() {
        ...
        if (jQuery(column).attr('name')=='bstable-actions') {
      ...
      $cols.each(function() {
        ...
        if (jQuery(column).attr('name')=='bstable-actions') {
          ...
          jQuery(column).html('');  // clear the text
        }
      });
    }
    this._addOnClickEventsToActions(); // Add onclick events to each action button in all rows
  ...
  // Go through each editable field and perform the howToModifyFunction function
    ...
    $cols.each(function() {
      n++;
      if (jQuery(this).attr('name')=='bstable-actions') return;    // exclude the actions column
      if (!isEditableColumn(n-1)) return;                     // Check if the column is editable
      howToModify(jQuery(this));                                   // If editable, call the provided function
    });
  ...
  // Convert table to CSV
    ...
    _this.table.find('tbody tr').each(function() {
        ...
        if (_this.currentlyEditingRow(jQuery(this))) {
            jQuery(this).find('#bAcep').click();       // Force Accept Edit
        }
        var $cols = jQuery(this).find('td');           // read columns
        rowValues = '';
        $cols.each(function() {
            if (jQuery(this).attr('name')=='bstable-actions') {
                // buttons column - do nothing
            } else {
                rowValues = rowValues + jQuery(this).html() + separator;
            }
        });
        if (rowValues!='') {
