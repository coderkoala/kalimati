...
@section('title', __('Update Trader Due Data'))

@inject('user', '\App\Domains\Auth\Models\User')

@section('content')
    <x-forms.patch :action="route('admin.traderdues.update', $data['id'])">
        <x-backend.card>
            <x-slot name="header">
                @lang('Update Trader Due Data')
            </x-slot>
            ...
            <x-slot name="footer">
                <button class="btn btn-sm btn-primary float-right" type="submit">@lang('Update Trader Due Data')</button>
            </x-slot>
        </x-backend.card>
    </x-forms.post>
@endsection
