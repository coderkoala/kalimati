                ...
                <div class="col-sm-12">
                    ...
                    <div class="banner-link">
                        <ul>
                            ...
                            <li>
                                <span class="active">{{ $notice_title }}</span>
                            </li>
                        </ul>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-md-12 col-md-push-3">
                    <div class="blog-detail">
						<h4>{{ $notice_title }} {{ __('Home Page') }}</h4>
                                    ...
                                    <tbody>
                                        @foreach ($notice as $noticeTuple)
                                            <tr>
                                                ...
                                                <td><a href="{{ route('frontend.pages.notice-show', $noticeTuple->id) }}">{{ __('View Notice') }}</a></td>
                                                <td>{{ __date($noticeTuple->published_at) }}</td>
                                                <td><a href="{{ $noticeTuple->url  }}" download><i class="fa fa-download" aria-hidden="true"></i> {{ __('Download') }}</a></td>
                                            </tr>
                                        @endforeach
                                        @if(1 === $tableRow)
                                        <tr>
                                            <td colspan="5">{{__('frontend_notice.notice_no_results_found')}}</td>
                                        </tr>
                                        @endif
                                    </tbody>
                                </table>
