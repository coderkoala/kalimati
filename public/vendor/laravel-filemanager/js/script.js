...
jQuery.fn.fab = function (options) {
  ...
  var toggler = jQuery('<a>')
    ...
    .append(jQuery('<i>').addClass('fas fa-plus'))
    .click(function () {
  ...
  options.buttons.forEach(function (button) {
    toggler.before(
      jQuery('<a>').addClass('fab-button fab-action')
        .attr('data-label', button.label)
        .attr('id', button.attrs.id)
        .append(jQuery('<i>').addClass(button.icon))
        .click(function () {
...
jQuery(document).ready(function ($) {
  ...
  actions.reverse().forEach(function (action) {
    jQuery('#nav-buttons > ul').prepend(
      jQuery('<li>').addClass('nav-item').append(
        jQuery('<a>').addClass('nav-link d-none')
          .attr('data-action', action.name)
          .attr('data-multiple', action.multiple)
          .append(jQuery('<i>').addClass('fas fa-fw fa-' + action.icon))
          .append(jQuery('<span>').text(action.label))
      )
    );
  });

  sortings.forEach(function (sort) {
    jQuery('#nav-buttons .dropdown-menu').append(
      jQuery('<a>').addClass('dropdown-item').attr('data-sortby', sort.by)
        .append(jQuery('<i>').addClass('fas fa-fw fa-' + sort.icon))
        .append(jQuery('<span>').text(sort.label))
        .click(function() {
          sort_type = sort.by;
          loadItems();
        })
    ...
    .done(function (response) {
      JSON.parse(response).forEach(function (message) {
        jQuery('#alerts').append(
          jQuery('<div>').addClass('alert alert-warning')
            .append(jQuery('<i>').addClass('fas fa-exclamation-circle'))
            .append(' ' + message)
        );
      });
    });
  ...
  if (usingWysiwygEditor()) {
    jQuery('#multi_selection_toggle').hide();
  }
});
...
jQuery('#multi_selection_toggle').click(function () {
  multi_selection_enabled = !multi_selection_enabled;

  jQuery('#multi_selection_toggle i')
...
jQuery('#to-previous').click(function () {
...
function toggleMobileTree(should_display) {
  if (should_display === undefined) {
    should_display = !jQuery('#tree').hasClass('in');
  }
  jQuery('#tree').toggleClass('in', should_display);
}

jQuery('#show_tree').click(function (e) {
  toggleMobileTree();
});

jQuery('#main').click(function (e) {
  if (jQuery('#tree').hasClass('in')) {
...
jQuery(document).on('click', '[data-display]', function() {
  show_list = jQuery(this).data('display');
  loadItems();
});

jQuery(document).on('click', '[data-action]', function() {
  window[jQuery(this).data('action')](jQuery(this).data('multiple') ? getSelectedItems() : getOneSelectedElement());
});
...
function toggleSelected (e) {
  ...
  var sequence = jQuery(e.target).closest('a').data('id');
...
function toggleActions() {
  ...
  jQuery('[data-action=use]').toggleClass('d-none', !(many_selected && only_file));
  jQuery('[data-action=rename]').toggleClass('d-none', !one_selected);
  jQuery('[data-action=preview]').toggleClass('d-none', !(many_selected && only_file));
  jQuery('[data-action=move]').toggleClass('d-none', !many_selected);
  jQuery('[data-action=download]').toggleClass('d-none', !(many_selected && only_file));
  jQuery('[data-action=resize]').toggleClass('d-none', !(one_selected && only_image));
  jQuery('[data-action=crop]').toggleClass('d-none', !(one_selected && only_image));
  jQuery('[data-action=trash]').toggleClass('d-none', !many_selected);
  jQuery('[data-action=open]').toggleClass('d-none', !one_selected || only_file);
  jQuery('#multi_selection_toggle').toggleClass('d-none', usingWysiwygEditor() || !many_selected);
  jQuery('#actions').toggleClass('d-none', selected.length === 0);
  jQuery('#fab').toggleClass('d-none', selected.length !== 0);
}
...
jQuery(document).on('click', '#tree a', function (e) {
  goTo(jQuery(e.target).closest('a').data('path'));
  toggleMobileTree(false);
});

function goTo(new_dir) {
  jQuery('#working_dir').val(new_dir);
  loadItems();
}

function getPreviousDir() {
  var working_dir = jQuery('#working_dir').val();
  return working_dir.substring(0, working_dir.lastIndexOf('/'));
}

function setOpenFolders() {
  jQuery('#tree [data-path]').each(function (index, folder) {
    // close folders that are not parent
    var should_open = (jQuery('#working_dir').val() + '/').startsWith(jQuery(folder).data('path') + '/');
    jQuery(folder).children('i')
      .toggleClass('fa-folder-open', should_open)
      .toggleClass('fa-folder', !should_open);
  });

  jQuery('#tree .nav-item').removeClass('active');
  jQuery('#tree [data-path="' + jQuery('#working_dir').val() + '"]').parent('.nav-item').addClass('active');
}
...
var hideNavAndShowEditor = function (data) {
  jQuery('#nav-buttons > ul').addClass('d-none');
  jQuery('#content').html(data);
  jQuery('#pagination').removeClass('preserve_actions_space')
  clearSelected();
}
...
function generatePaginationHTML(el, args) {
  ...
  if (args.pageRange === null) {
    for (i = 1; i <= totalPage; i++) {
      var button = jQuery(template)
        .attr('data-num', i)
        .append(jQuery(linkTemplate).html(i));
      if (i == currentPage) {
  ...
  if (rangeStart <= 3) {
    ...
    var button = jQuery(template)
      .attr('data-num', 1)
      .append(jQuery(linkTemplate).html(1));
    el.append(button);

    var button = jQuery(template)
      .addClass('disabled')
      .append(jQuery(linkTemplate).html('...'));
    el.append(button);
  }
  ...
  if (rangeEnd >= totalPage - 2) {
    ...
    var button = jQuery(template)
      ...
      .append(jQuery(linkTemplate).html(totalPage));
    el.append(button);
  }
}

function createPagination(paginationSetting) {
  var el = jQuery('<ul class="pagination" role="navigation"></ul>');
...
function loadItems(page) {
  ...
  performLfmRequest('jsonitems', {show_list: show_list, sort_type: sort_type, page: page || 1}, 'html')
    .done(function (data) {
      ...
      jQuery('#empty').toggleClass('d-none', hasItems);
      jQuery('#content').html('').removeAttr('class');
      jQuery('#pagination').html('').removeAttr('class');

      if (hasItems) {
        jQuery('#content').addClass(response.display);
        jQuery('#pagination').addClass('preserve_actions_space');

        items.forEach(function (item, index) {
          var template = jQuery('#item-template').clone()
            .removeAttr('id class')
            .attr('data-id', index)
          ...
          if (item.thumb_url) {
            var image = jQuery('<div>').css('background-image', 'url("' + item.thumb_url + '?timestamp=' + item.time + '")');
          } else {
            var icon = jQuery('<div>').addClass('ico');
            var image = jQuery('<div>').addClass('mime-icon ico-' + item.icon).append(icon);
          }
      ...
      if (hasPaginator) {
        ...
        jQuery('#pagination a').on('click', function(event) {
          ...
          loadItems(jQuery(this).closest('li')[0].getAttribute('data-num'));

          return false;
        });
      }

      jQuery('#nav-buttons > ul').removeClass('d-none');

      jQuery('#working_dir').val(working_dir);
      ...
      validSegments.forEach(function (segment, index) {
        if (index === 0) {
          ...
          breadcrumbs.push(jQuery("[data-path='/" + segment + "']").text());
        } else {
          breadcrumbs.push(segment);
        }
      });

      jQuery('#current_folder').text(breadcrumbs[breadcrumbs.length - 1]);
      jQuery('#breadcrumbs > ol').html('');
      breadcrumbs.forEach(function (breadcrumb, index) {
        var li = jQuery('<li>').addClass('breadcrumb-item').text(breadcrumb);
        ...
        jQuery('#breadcrumbs > ol').append(li);
      });

      var atRootFolder = getPreviousDir() == '';
      jQuery('#to-previous').toggleClass('d-none invisible-lg', atRootFolder);
      jQuery('#show_tree').toggleClass('d-none', !atRootFolder).toggleClass('d-block', atRootFolder);
      setOpenFolders();
      loading(false);
      toggleActions();
    });
...
function loading(show_loading) {
  jQuery('#loading').toggleClass('d-none', !show_loading);
}
...
function download(items) {
  items.forEach(function (item, index) {
    ...
    setTimeout(function () {
      location.href = lfm_route.replace(/\/+$/, '') + '/download?' + jQuery.param(data);
    }, index * 100);
  });
}
...
function preview(items) {
  var carousel = jQuery('#carouselTemplate').clone().attr('id', 'previewCarousel').removeClass('d-none');
  ...
  items.forEach(function (item, index) {
    ...
    if (item.thumb_url) {
      ...
      carouselItem.find('.carousel-image').css('width', '50vh').append(jQuery('<div>').addClass('mime-icon ico-' + item.icon));
    }

    carouselItem.find('.carousel-label').attr('target', '_blank').attr('href', item.url)
      .append(item.name)
      .append(jQuery('<i class="fas fa-external-link-alt ml-2"></i>'));
...
function defaultParameters() {
  return {
    working_dir: jQuery('#working_dir').val(),
...
function notify(body, callback) {
  jQuery('#notify').find('.btn-primary').toggle(callback !== undefined);
  jQuery('#notify').find('.btn-primary').unbind().click(callback);
  jQuery('#notify').modal('show').find('.modal-body').html(body);
}

function dialog(title, value, callback) {
  jQuery('#dialog').find('input').val(value);
  jQuery('#dialog').on('shown.bs.modal', function () {
    jQuery('#dialog').find('input').focus();
  });
  jQuery('#dialog').find('.btn-primary').unbind().click(function (e) {
    callback(jQuery('#dialog').find('input').val());
  });
  jQuery('#dialog').modal('show').find('.modal-title').text(title);
}
